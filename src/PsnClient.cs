	...
	public class PsnClient : IDisposable
	{
		...
		protected virtual void OnInfoPacketReceived(PsnInfoPacketChunk infoPacket)
		{
			...
			if (_currentInfoPacketChunks.Any())
			{
				...
				{
					InvalidPacketReceived?.Invoke(this, new InvalidPacketsReceivedEventArgs(_currentInfoPacketChunks, false,
			...
			IReadOnlyCollection<PsnInfoPacketChunk> infoPackets)
		{
			var infoTrackerChunks = infoPackets.SelectMany(p =>
				((PsnInfoTrackerListChunk)p.SubChunks.Single(c => c.ChunkId == PsnInfoPacketChunkId.PsnInfoTrackerList)).SubChunks)
				.ToList();
		...
		protected virtual void OnDataPacketReceived(PsnDataPacketChunk dataPacket)
		{
			...
			{
				InvalidPacketReceived?.Invoke(this,
					new InvalidPacketsReceivedEventArgs(dataPacket, false, "Packet contains multiple data packet header chunks"));
				return;
			}

			var headerChunk =
				(PsnDataHeaderChunk)dataPacket.SubChunks.FirstOrDefault(c => c.ChunkId == PsnDataPacketChunkId.PsnDataHeader);
			...
			IReadOnlyCollection<PsnDataPacketChunk> dataPackets)
		{
			var dataTrackerChunks = dataPackets.SelectMany(p =>
				((PsnDataTrackerListChunk)p.SubChunks.Single(c => c.ChunkId == PsnDataPacketChunkId.PsnDataTrackerList)).SubChunks)
				.ToList();
		...
		public class InvalidPacketsReceivedEventArgs : EventArgs
		{
			...
			internal InvalidPacketsReceivedEventArgs(IEnumerable<PsnPacketChunk> packets, bool wasProcessed, string message)
			{
