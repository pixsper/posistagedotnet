...
namespace DBDesign.PosiStageDotNet.Serialization
{
	...
	internal abstract class EndianBitConverter
	{
		...
		{
			return getBytesImpl(DoubleToInt64Bits(value), 8);
		}
		...
		{
			return getBytesImpl(SingleToInt32Bits(value), 4);
		}
		...
		{
			return getBytesImpl(unchecked((long)value), 8);
		}
		...
		{
			copyBytesImpl(value ? 1 : 0, 1, buffer, index);
		}
		...
		{
			copyBytesImpl(DoubleToInt64Bits(value), 8, buffer, index);
		}
		...
		{
			copyBytesImpl(value, 8, buffer, index);
		}
		...
		{
			copyBytesImpl(SingleToInt32Bits(value), 4, buffer, index);
		}
		...
		{
			copyBytesImpl(unchecked((long)value), 8, buffer, index);
		}
		...
		byte[] getBytesImpl(long value, int bytes)
		{
			var buffer = new byte[bytes];
			copyBytesImpl(value, bytes, buffer, 0);
			return buffer;
		}
		...
		/// <param name="buffer">The byte array to copy the bytes into</param>
		/// <param name="index">The first index into the array to copy the bytes into</param>
		void copyBytesImpl(long value, int bytes, byte[] buffer, int index)
		{
